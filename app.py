...
except: encoded_nav_logo = ""

st.markdown(f"""<div class="navbar"><div class="navbar-logo"><img src="data:image/png;base64,{encoded_nav_logo}" width="32" height="32" style="object-fit:cover;"></div><span class="navbar-title">Universitas Maritim Raja Ali Haji</span></div>""", unsafe_allow_html=True)
st.markdown("<div class='page-wrapper'>", unsafe_allow_html=True)
st.markdown("""<div class="welcome-container"><div class="welcome-title">Selamat Datang di<br>Sistem Klasifikasi Gonggong</div><div class="welcome-subtitle">Identifikasi jenis siput Gonggong khas Kepulauan Riau secara instan dan akurat.</div><a class='cta-scroll-button' href='#mulai-klasifikasi'>Klasifikasi Sekarang</a></div><div id='mulai-klasifikasi'></div>""", unsafe_allow_html=True)
...
def application_core():
    ...
    if uploaded_file is None:
        ...
        with col1:
            st.markdown(f"<div class='img-preview-container'>{st.image(image, use_column_width=True)}</div>", unsafe_allow_html=True)
        with col2:
            if st.session_state.bg_removed_image:
                st.markdown(f"<div class='img-preview-container'>{st.image(st.session_state.bg_removed_image, use_column_width=True)}</div>", unsafe_allow_html=True)
            else:
        ...
        if st.session_state.bg_removed_image is None:
            ...
            c1, c2 = st.columns(2)
            with c1:
                if st.button("Ulangi", key="core_btn_reset"):
                    st.session_state.bg_removed_image = None
                    st.rerun()
            with c2:
                if st.button("Analisis Gambar", key="core_btn_anlz"):
                    ...
                    if conf < 0.50:
                        st.session_state.warn_html = "<div class='warning-box'>⚠️ Tidak dikenali.</div>"
                    else:
                        st.session_state.pred_class = class_names[idx]
                        st.session_state.conf_text = f"{conf * 100:.2f} %"
                    st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)

    if st.session_state.warn_html: st.markdown(st.session_state.warn_html, unsafe_allow_html=True)
    st.markdown(f"<div class='result-box'><span class='result-label'>Hasil :</span><span class='result-value'>{st.session_state.pred_class}</span></div>", unsafe_allow_html=True)
...
# Tambahkan spasi agar file tetap panjang
for i in range(20): st.write("")
application_core()
st.markdown("</div>", unsafe_allow_html=True)
st.markdown("<div class='white-footer-canvas'><div class='footer-text'>© 2026 CNN MobileNet<br>UMRAH</div></div>", unsafe_allow_html=True)

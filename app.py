...
def application_core():
    ...
    if uploaded_file is None:
        ...
        with col1:
            ...
            st.markdown(f"<div class='img-preview-container'><img src='data:image/jpeg;base64,{img_str1}'></div><div style='text-align:center; font-weight:600;'>Gambar Asli</div>", unsafe_allow_html=True)
        with col2:
            if st.session_state.bg_removed_image:
                ...
                st.markdown(f"<div class='img-preview-container'><img src='data:image/jpeg;base64,{img_str2}'></div><div style='text-align:center; font-weight:600;'>Hasil Hapus Background</div>", unsafe_allow_html=True)
            else:
        ...
        if st.session_state.bg_removed_image is None:
            ...
            c1, c2 = st.columns(2)
            with c1:
                if st.button("Proses Ulang", key="core_btn_reset"):
                    st.session_state.bg_removed_image = None
                    st.rerun()
            with c2:
                if st.button("Analisis Gambar", key="core_btn_anlz"):
                    # PREPROCESSING AKURAT
                    ...
                    max_conf = prediction[0][best_idx]
                    ...
                    if max_conf < 0.50:
                        ...
                        st.session_state.warn_html = ""
                        st.session_state.pred_class = class_names[best_idx].replace('_', ' ')
...
application_core()
st.markdown("</div>", unsafe_allow_html=True) 
st.markdown("""<div class='white-footer-canvas'><div class='footer-text'>© 2026 Sistem Klasifikasi Jenis Gonggong berbasis CNN MobileNet<br>Fakultas Teknik dan Teknologi Kemaritiman - UMRAH</div></div></div>""", unsafe_allow_html=True)
